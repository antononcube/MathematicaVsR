  ...
  cat( "\tTime to compute the NMF of training images sub-matrices :", system.time( {
    nnmfRes <- 
      llply( classLabels, function(cl) {
        ...
        smat <- trainImagesMat[ inds, ]
        smat <- as( smat, "sparseMatrix")
        res <- NNMF( V = smat, k = 40, maxSteps = 200, tolerance = 1E-4, regularizationParameter = 0.1 )
...
NNMFClassifyImageVector <- function( factorizationsRes, vec, positiveDifference = FALSE ) {
  
  residuals <- 
    laply( factorizationsRes, function(x){
      approxVec <- t(x$H) %*% (Diagonal(x = x$D) %*% ( x$H %*% vec ) )
      rv <- vec / vnorm(vec) - approxVec[,1] / vnorm( approxVec[,1] ) 
      if ( positiveDifference ) { rv <- ifelse( rv < 0, 0, rv ) }
      vnorm(rv)
    })
  names(residuals) <- names(factorizationsRes)
...
```{r}
cat( "\tTime to compute the NMF classifier evalution :", system.time( {
  nnmfClRes <- 
      ldply( 1:nrow(testImagesMat), function( i ) {
        res <- NNMFClassifyImageVector( factorizationsRes = nnmfRes, vec = testImagesMat[i,], positiveDifference = FALSE )
        data.frame( Actual = testImagesLabels[[i]], Predicted = res$Label, stringsAsFactors = FALSE )
      } )
}), "\n")
```
