...
<!--- 
rmarkdown::render( input = "./SummarizationOfFreakonomicsPodcasts.Rmd", output_file = "./SummarizationOfFreakonomicsPodcasts.html")
--->
...
In order to those experiments we have find, choose, and download suitable text data. This document gives a walk through with code of the complete sequence of steps from intent to experimental results. 
...
4. Mapping of selected document into linear vector space using the bag-of-words model.

5. Finding sentence saliency using matrix algebra.
...
The following code goes through each podcast links dowloads its content.
...
Each of the obtained podcast transcripts are respresented as list of stings, each string corresponding to a paragraph in the transcript HTML file.
...
Using the transcript term matrix we can find additional stop words that specifica for the transcript collection. We can ask which words have frequencies comparable with the stop words. These commands find the top most frequent words in the collection:
...
At this point we are ready to program a function that is going calculate the most important statements in a podcast using the algegraic approach explained in \[5\] by Elden.
...
```{r}
podcastTitles[[96]]
MostImportantSentences( sentences = podcastTexts[[96]], nSentences = 5, stopWords = stopWords, applyWordStemming = FALSE )
```

```{r}
podcastTitles[[122]]
MostImportantSentences( sentences = podcastTexts[[122]], nSentences = 5, stopWords = stopWords, applyWordStemming = FALSE )
```


### Interactive interface

Running the file "SalientStatementsInterface.R" would produce a Shiny intractive interface that allows to see effects of different parameter combinations.

## References

\[1\] Anton Antonov, [MathematicaVsR project at GitHub](https://github.com/antononcube/MathematicaVsR).

\[2\] Anton Antonv, ["Implementation of document-term matrix re-weighting functions in R"](https://github.com/antononcube/MathematicaForPrediction/blob/master/R/DocumentTermWeightFunctions.R) (2014) at [MathematicaForPrediction project at GitHub](https://github.com/antononcube/MathematicaForPrediction).
